...
- Para Mesa STEM, sustituir `.book-placeholder` por la imagen real `assets/img/mesa-stem/portada.jpg` siguiendo el comentario HTML. El botón “Ver libro” conserva `href="#"` y está deshabilitado mientras no exista una URL oficial. Al añadirla, retirar `aria-disabled`, `tabindex`, `aria-describedby`, la clase `pending-link` y el aviso `#book-link-status`. No publicar documentación ni información sensible de proyectos institucionales.
